package com.exemple.ecommerce.ui.gateway.location;
    ...
    public void location201Rewrite() {
        ...
        apiClient.when(HttpRequest.request().withMethod("GET").withPath("/ExempleEcommerce/info"))
                .respond(HttpResponse.response().withHeaders(new Header("Content-Type", "application/json;charset=UTF-8"),
                        new Header("Location", "http://localhost:" + apiPort + "/ExempleEcommerce/123")).withStatusCode(201));
        ...
        assertThat(response.getHeader("Location"), is(restTemplate.getRootUri() + "/ExempleEcommerce/123"));
